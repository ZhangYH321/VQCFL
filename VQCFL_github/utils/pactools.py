...
def average_weights_weighted(w, avg_weight,train_keys):
    """
...
def as_average_weights_weighted(w_dict, avg_weights, train_keys):
    """
    Returns the weighted average of the weights.
    Args:
        w_dict: Dictionary of client weights {client_idx: state_dict}
        avg_weights: List of weights for averaging
        train_keys: Keys to use for averaging
    """
    # Get first client's weights as template
    first_client = list(w_dict.keys())[0]
    w_avg = copy.deepcopy(w_dict[first_client])

    # Convert weights to tensor and normalize
    weight = torch.tensor(avg_weights)
    agg_w = weight / weight.sum()

    # Average each key
    for key in train_keys:
        # Initialize with zeros of the same shape and device as the weights
        w_avg[key] = torch.zeros_like(w_dict[first_client][key])

        # Weighted sum for each client
        for i, client_idx in enumerate(w_dict.keys()):
            w_avg[key] += agg_w[i] * w_dict[client_idx][key]
...
def get_head_agg_weight(num_users, Vars, Hs, *args, **kwargs):
    ...
    for i in range(num_users):
        ...
        ## 远程用这个
        evals_complex, evecs_complex = torch.linalg.eig(torch.tensor(p_matrix))
        evals = evals_complex.real
        evecs = evecs_complex.real
        # for numerical stablity
        p_matrix_new = 0
        for ii in range(num_users):
            if evals[ii] >= 0.01:
                p_matrix_new += evals[ii] * torch.mm(evecs[:, ii].reshape(num_users, 1),
